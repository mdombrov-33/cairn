"""Scene birth — turning an authored (or thin) location into a live Scene row.
...
# The AuthoredScene keys carried through from the raw YAML — the read-mostly bucket.
# scene_mode / safety_level drive columns; npcs_present + npc_agendas_in_scene become
# runtime presence; the rest (id, location_id, time_of_day) is authoring metadata.
_AUTHORED_KEYS = ("atmosphere", "surface_details", "hidden", "secrets", "threads_in_air", "hooks_out")


def _split_authored(raw: dict[str, Any]) -> AuthoredScene:
    """Pick the read-mostly authored bucket out of the raw scene dict."""
    return {k: raw[k] for k in _AUTHORED_KEYS if k in raw}  # type: ignore[return-value]
